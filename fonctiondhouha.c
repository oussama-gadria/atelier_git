...
void trouver_etudiant(GtkWidget* liste, char f[])
{
	int i;
	GtkCellRenderer *renderer;
	GtkTreeViewColumn *column;
	...
	for(i=0;i<ne;i++)
	{ printf("%d",strlen(f));
		if (strncmp(f,TE[i].identifiantE,strlen(f))==0)
		{

		gtk_list_store_append(store, &iter);
		gtk_list_store_set(store, &iter, ENOME, TE[i].nomE, EPRENOME, TE[i].prenomE,EGENRE, TE[i].genre, EIDENTIFIANTE, TE[i].identifiantE,EETAB,TE[i].etablissement ,ECHAMBRE , TE[i].num_chambre, EETAGE, TE[i].num_etage,ENIVEAU, TE[i].niveau,-1);
	}
	}
  ...
  int filesave_etudiant()
{
    ...
    for(i=0;i<ne;i++)
    {
        fprintf(f,"%s %s %s %s %s %s %s %d\n",TE[i].nomE,TE[i].prenomE,TE[i].genre,TE[i].identifiantE,TE[i].etablissement,TE[i].num_chambre,TE[i].num_etage,TE[i].niveau);
    }
    fclose(f);
    return 1;
...
{  
    ...
    while(fscanf(f,"%s %s %s %s %s %s %s %d\n",TE[ne].nomE,TE[ne].prenomE,TE[ne].genre,TE[ne].identifiantE,TE[ne].etablissement,TE[ne].num_chambre,TE[ne].num_etage,&TE[ne].niveau)!=EOF)
    {
        ne++;
    }
